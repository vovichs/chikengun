...
namespace UnityStandardAssets.Characters.FirstPerson
{
	...
	public class RigidbodyFirstPersonController : MonoBehaviour
	{
		...
		{
			if (Physics.SphereCast(base.transform.position, m_Capsule.radius * (1f - advancedSettings.shellOffset), Vector3.down, out RaycastHit hitInfo, m_Capsule.height / 2f - m_Capsule.radius + advancedSettings.stickToGroundHelperDistance, -1, QueryTriggerInteraction.Ignore) && Mathf.Abs(Vector3.Angle(hitInfo.normal, Vector3.up)) < 85f)
			{
		...
		private void RotateView()
		{
			if (!(Mathf.Abs(Time.timeScale) < float.Epsilon))
			{
				...
				if (m_IsGrounded || advancedSettings.airControl)
				{
					...
					Quaternion rotation = Quaternion.AngleAxis(eulerAngles2.y - y, Vector3.up);
		...
		private void GroundCheck()
		{
			...
			if (Physics.SphereCast(base.transform.position, m_Capsule.radius * (1f - advancedSettings.shellOffset), Vector3.down, out RaycastHit hitInfo, m_Capsule.height / 2f - m_Capsule.radius + advancedSettings.groundCheckDistance, -1, QueryTriggerInteraction.Ignore))
			{
				m_IsGrounded = true;

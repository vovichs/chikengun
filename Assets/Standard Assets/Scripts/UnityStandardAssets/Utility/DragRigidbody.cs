...
namespace UnityStandardAssets.Utility
{
	public class DragRigidbody : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			if (Physics.Raycast(camera.ScreenPointToRay(UnityEngine.Input.mousePosition).origin, camera.ScreenPointToRay(UnityEngine.Input.mousePosition).direction, out hitInfo, 100f, -5) && (bool)hitInfo.rigidbody && !hitInfo.rigidbody.isKinematic)
			{
				if (!m_SpringJoint)
				{
